...
describe("buffer exceeded", function () {
    var output = null;

    before(function (done) {
        parseApk(__dirname + "/samples/buffer_exceeded.apk", 1024 * 1024 * 5, function (err, out) {
            if (err) {
                return done(err);
            }
            output = out;
    ...
    it("Has a package name", function () {
        assert.equal(output.manifest[0]["@package"], "com.google.android.talk");
    });
    ...
    it("Has an icon", function () {
        var manifest = output.manifest[0];
        assert.equal(manifest.application[0]["@android:icon"], "product_logo_hangouts_launcher_color_48");
    });
});
